...
struct AuthField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 7) {
                if groupedLoginStyle {
                    Image(systemName: icon).foregroundStyle(Brand.red).accessibilityHidden(true)
                }
                Text(title).foregroundStyle(groupedLoginStyle ? Color.secondary : Color.primary)
            ...
            .frame(minHeight: 52)
            .background(Color(uiColor: groupedLoginStyle ? .tertiarySystemGroupedBackground : .secondarySystemGroupedBackground), in: RoundedRectangle(cornerRadius: 8))
            .overlay {
                RoundedRectangle(cornerRadius: 8)
                    .strokeBorder(
                        error != nil ? Color.red : focused ? Brand.red : Color(uiColor: .separator).opacity(0.55),
